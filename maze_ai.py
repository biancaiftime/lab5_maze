...
class Graf:
    ...
    def calculeaza_h(self, reprez_nod):
        ind_unu_in_reprez_nod = reprez_nod.index(1)
        ind_unu_in_sco_nod = self.reprez_scop.index(1)
        i_scop , j_scop = self.get_x_y(ind_unu_in_sco_nod)
        i_nod, j_nod = self.get_x_y(ind_unu_in_reprez_nod)
        return abs(i_scop-i_nod)+abs(j_nod-j_scop)
    ...
    def calculeaza_succesori(self, nod):
        poz_unu_in_nod = nod.info.index(1)
        i = poz_unu_in_nod // 20
        j = poz_unu_in_nod % 20
        succesori = []
        if i >= 1:
            poz_unu_in_urma_mutarii = poz_unu_in_nod - 20
            if nod.info[poz_unu_in_urma_mutarii] != 2:
                lista_nou = self.interschimba(poz_unu_in_nod, poz_unu_in_urma_mutarii, nod.info.copy())
                h_fiu = self.calculeaza_h(lista_nou)
                succesori.append((Nod(lista_nou, h_fiu), 1))
        if i <= 18:
            poz_unu_in_urma_mutarii = poz_unu_in_nod + 20
            if nod.info[poz_unu_in_urma_mutarii] != 2:
                lista_nou = self.interschimba(poz_unu_in_nod, poz_unu_in_urma_mutarii, nod.info.copy())
        ...
        if j >= 1:
            poz_unu_in_urma_mutarii = poz_unu_in_nod - 1
            if nod.info[poz_unu_in_urma_mutarii] != 2:
                lista_nou = self.interschimba(poz_unu_in_nod, poz_unu_in_urma_mutarii, nod.info.copy())
                h_fiu = self.calculeaza_h(lista_nou)
                succesori.append((Nod(lista_nou, h_fiu), 1))
