...
def draw_game_window(pozitii, scop):
    for ind in range(len(pozitii)):
        ...
        i_elem = ind % 20
        if pozitii[ind] == 0:
            win.blit(number_list[2], (i_elem * 25, j_elem * 25)) #*pot sa mut*
        elif pozitii[ind] == 1:
            win.blit(number_list[0], (i_elem * 25, j_elem * 25)) #*casuta mobila*
        elif pozitii[ind] == 2:
            win.blit(number_list[3], (i_elem * 25, j_elem * 25)) #*zid*
    ind = scop.index(1)
    j_elem = ind // 20
    i_elem = ind % 20
